...
class MarkdownHeaderProcessor:
    def __init__(self, api_key=None, rename_files=True, rename_only_new=True, model="google/gemini-2.5-pro-preview-03-25"):
        ...
        if not api_key:
            print("Warning: No OpenRouter API key provided. LLM-based features will be disabled.")
    ...
    def generate_tags(self, content, title):
        ...
        if response:
            ...
            suggested_tags = [tag.strip() for tag in response.strip().split('\n') if tag.strip()]
            ...
            if valid_tags:
                ...
                print("No valid tags returned from API. Skipping tag generation.")
                return None
...
def main():
    ...
    parser.add_argument('--model', help='Model to use (default: google/gemini-2.5-pro-preview-03-25)', 
